...
lt <- apply(qvalue, 2, function(x){rownames(qvalue)[x<0.05]})
...
pdf("../result/Sfig/sfig8a.acarbose.compare.upset.pdf", width = 15, height = 4)
ht = UpSet(m,
           #set_order = order(ss),
...
p <- ComplexHeatmap::pheatmap(as.matrix(qdat2)[, c(6:8,1:5)],
                              display_numbers = qdatNum[,c(6:8,1:5)],
                              cluster_rows = F,  color =colt,
                              cluster_cols = F, fontsize_number = 20,
                              number_color = "White", angle_col = "90",
                              annotation_row  = con_acar[rownames(qdat2), ,drop = F],
                              annotation_colors = list(V2 = c(negative = "#1B9E77", positive = "#D95F02")),
                              gaps_col = c(5))

pdf("../result/Sfig/sfig8b.acar.spe.pdf", width = 5, height = 8)
print(p)
dev.off()
