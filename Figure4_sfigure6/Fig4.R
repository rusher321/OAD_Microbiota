#------------------------------- Fig4 --------------------------------------#
# figa 
qdat1_melt <- readRDS("../inputdata/fig4_diversity.rds")
qdat1_melt$group <- gsub("YangFM_2021_Metformin", "RenHH_2023_Metformin", qdat1_melt$group)
qdat1_melt$group <- factor(qdat1_melt$group,
                           levels = c("GuYY_2017_Acarbose", "ZhangXY_2021_Acarbose", "ZhangYF_2020_Berberine",
                                      "RenHH_2023_Metformin", "ZhangXY_2021_Vlidagliptin" ,"GuYY_2017_Glipizide", "ZhangYF_2020_Placebo"))
qdat1_melt$var <- factor(qdat1_melt$var, levels = rev(c("HbA1c", "FPG", "PPG120", "Ins0", "Ins120", "HOMA_IR")))

pcor_plot2 <- ggplot(qdat1_melt, aes(x= variable, y = var, fill= rho)) +
  geom_tile() +
  scale_fill_distiller(palette = "RdBu") +
  theme_ipsum(base_family = "Helvetica")+
  geom_text(label = qdat2_melt$pvalue, size = 2.5)+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+facet_grid(.~group)

ggsave(filename = "../result/Figure4/Fig4a.pdf",
       plot = pcor_plot2, device = "pdf", width = 12, height = 4)
...
ggsave(filename = "../result/Figure4/Fig4b_left.pdf", qdat_fig,
       device = "pdf", width = 6, height = 4)

outplot <- readRDS("../inputdata/fig4_r2.rds")
...
ggsave(filename = "../result/Figure4/Fig4b_right.pdf", out_fig,
       device = "pdf", width = 6, height = 4)
...
ggsave(filename = "../result/Figure4/fig4c.pdf", plot = out,
...
ggsave(filename = "../result/Figure4/fig4_Bacteria.or.pdf", plot = p, device = "pdf", width = 6, height = 4)
